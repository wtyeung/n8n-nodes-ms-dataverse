...
**New Features:**
- **Share Access Operations**: Complete access control management
  - Share Access Add: Grant access to users/teams with configurable permissions
  - Share Access List: View all principals with access to a record
  - Share Access Revoke: Remove access from users/teams
  - Support for UPN/email and team name lookup (automatic GUID resolution)
  - All share operations support both primary key ID and alternate keys
- **Upsert Operation**: Create or update records based on alternate keys
  - PATCH with alternate keys for true upsert behavior
  - POST without alternate keys to always create new records
  - Supports both JSON and field collection input modes
- **Dynamic Field Selection**: Field names load from table metadata
  - Dropdowns show display name, logical name, and type
  - Works for Create, Update, and Upsert operations
- **Alternate Key Dropdown**: Key names load from table's defined alternate keys
  - Shows only fields that are part of alternate keys
  - Prevents typos and improves UX
  - Works across Get, Update, Delete, and Share operations
